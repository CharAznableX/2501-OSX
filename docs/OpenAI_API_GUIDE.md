...
2. **Performance**: The first request to a local model loads it into memory; later requests can reuse the loaded runtime. Generation settings such as prefill step size, max context length, and KV-related options are auto-tuned when not explicitly configured in Settings. `cache_hint` / `prefix_hash` remain the API-facing cache-key mechanism, but actual reuse behavior depends on the active local engine. On the current VMLX path, cache lookup/store happens inside the runtime during generation rather than through a separate UI warm-up-only phase.

3. **Memory Management**: Models are loaded on demand and may be unloaded automatically when no active window still references them. The current native VMLX path uses a layered cache strategy with paged KV reuse, RAM cache, disk cache, and hybrid SSM companion state. Legacy MLX-managed models still use the older `ModelRuntime` / `KVCacheStore` behavior.
...
5. **Context Length**: Each model has different context limits. Settings in Local Inference still control max context and cache-related knobs, but the exact runtime behavior now depends on whether the request is being served by the native VMLX path or the legacy MLX path.

    ...
    public actor SandboxManager {
            ...
            results.append(
                await diagnose("vsock-bridge") {
                    let r = try await exec(
                        command: "curl -sf --unix-socket /tmp/project2501-bridge.sock http://localhost/api/log "
        ...
        private func ensureKernel() async throws -> Kernel {
            ...
            let extractDir = FileManager.default.temporaryDirectory.appendingPathComponent(
                "project2501-kernel-\(UUID().uuidString)"
            )
        ...
        private func configureSandbox() async throws {
            ...
            _ = try await execAsRoot(
                command:
                    "cp /workspace/.project2501-host-shim /usr/local/bin/project2501-host && chmod 555 /usr/local/bin/project2501-host && rm /workspace/.project2501-host-shim"
            )
        }

        // MARK: - project2501-host Shell Shim

        private static let project2501HostShimScript = """
            #!/bin/sh
            # project2501-host — Host API bridge shim for sandbox plugins.
            ...
            _call() {
              ...
              if [ "$_code" -ge 400 ] 2>/dev/null || [ -z "$_code" ]; then
                ...
                rm -f "$_tmp"
                echo "project2501-host: error ${_code:-000}: ${_err:-request failed}" >&2
                exit 1
              fi
              cat "$_tmp"
              rm -f "$_tmp"
            ...
            case "$1" in
              secrets)
                case "$2" in
                  ...
                  *) echo "Usage: project2501-host secrets get <name>" >&2; exit 1 ;;
                esac ;;
              config)
                case "$2" in
                  get) _call $H "$API/config/$3" | jq -r '.value // empty' ;;
                  set) _call -X POST $H "$API/config/$3" -d "{\\"value\\":\\"$4\\"}" > /dev/null ;;
                  *) echo "Usage: project2501-host config get|set <key> [value]" >&2; exit 1 ;;
                esac ;;
              inference)
                case "$2" in
                  chat)
                  ...
                  *) echo "Usage: project2501-host inference chat -m <message>" >&2; exit 1 ;;
                esac ;;
              agent)
                case "$2" in
                  ...
                  memory)
                    case "$3" in
                      ...
                      *) echo "Usage: project2501-host agent memory query|store <text>" >&2; exit 1 ;;
                    esac ;;
                  *) echo "Usage: project2501-host agent dispatch|memory ..." >&2; exit 1 ;;
                esac ;;
              events)
                case "$2" in
                  emit) _call -X POST $H "$API/events/emit" -d "{\\"type\\":\\"$3\\",\\"payload\\":${4:-{}}}" > /dev/null ;;
                  *) echo "Usage: project2501-host events emit <type> [payload]" >&2; exit 1 ;;
                esac ;;
              plugin)
                case "$2" in

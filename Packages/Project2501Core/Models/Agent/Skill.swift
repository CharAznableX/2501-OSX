...
extension Skill {
    ...
    private func toAgentSkillsFormatInternal(includeId: Bool) -> String {
        ...
        yaml += "metadata:\n"
        if includeId {
            yaml += "  project2501-id: \"\(id.uuidString)\"\n"
            yaml += "  project2501-enabled: \(enabled)\n"
        }
        if let pluginId = pluginId {
            yaml += "  project2501-plugin-id: \"\(pluginId)\"\n"
        }
        if let author = author {
    ...
    public static func parseAgentSkillsFormat(from markdown: String) throws -> Skill {
        ...
        if let metadata = frontmatter["metadata"] as? [String: Any] {
            ...
            if let enabledValue = metadata["project2501-enabled"] as? Bool {
                enabled = enabledValue
            }
            pluginId = metadata["project2501-plugin-id"] as? String
        }

                                ...
                                VStack(alignment: .leading, spacing: 20) {
                                    ...
                                    SettingsSubsection(label: "Command Line Tool") {
                                        VStack(alignment: .leading, spacing: 12) {
                                            Text("Install the `project2501` CLI into your PATH for terminal access.")
                                                .font(.system(size: 12))
                                                .foregroundColor(theme.tertiaryText)
                                ...
                                VStack(alignment: .leading, spacing: 20) {
                                    ...
                                    SettingsSubsection(label: "Tools") {
                                        VStack(alignment: .leading, spacing: 8) {
                                            ...
                                            Text(
                                                "Send messages directly to the model with no tool specs or capability injection. Keeps the prompt stable across turns for maximum KV-cache reuse. Recommended when project2501 is acting as a backend for an external agent."
                                            )
                                            .font(.system(size: 11))
                                            .foregroundColor(theme.tertiaryText)
                                        }
...
extension ConfigurationView {
    private func installCLI() {
        ...
        if tryInstall(cliURL: cliURL, into: brewBin) {
            ...
            cliInstallMessage = "Installed to \(brewBin.appendingPathComponent("project2501").path)"
            return
        }

        if tryInstall(cliURL: cliURL, into: usrLocalBin) {
            cliInstallSuccess = true
            cliInstallMessage = "Installed to \(usrLocalBin.appendingPathComponent("project2501").path)"
            return
        }
        ...
        if tryInstall(cliURL: cliURL, into: userLocalBin) {
            let linkPath = userLocalBin.appendingPathComponent("project2501").path
    ...
    private func resolveCLIExecutableURL() -> URL? {
        ...
        let helpers = appURL.appendingPathComponent("Contents/Helpers/project2501", isDirectory: false)
        ...
        let macOS = appURL.appendingPathComponent("Contents/MacOS/project2501", isDirectory: false)
        ...
        // Check for project2501-cli binary (the actual CLI product name)
        let debugCLI = productsDir.appendingPathComponent("project2501-cli", isDirectory: false)
        ...
        let releaseCLI = releaseDir.appendingPathComponent("project2501-cli", isDirectory: false)
        ...
        let releaseAppHelpers =
            releaseDir
            .appendingPathComponent("project2501.app/Contents/Helpers/project2501", isDirectory: false)
        ...
        let releaseAppMacOS =
            releaseDir
            .appendingPathComponent("project2501.app/Contents/MacOS/project2501", isDirectory: false)
    ...
    private func tryInstall(cliURL: URL, into dir: URL) -> Bool {
        ...
        let linkURL = dir.appendingPathComponent("project2501")

...
public struct ToolsCreate {
    public static func execute(args: [String]) {
        guard let name = args.first, !name.isEmpty else {
            fputs("Usage: project2501 tools create <name> [--language swift|rust]\n", stderr)
            exit(EXIT_FAILURE)
        }
        var language = "swift"
    ...
    private static func createPluginConfig(dir: URL, pluginId: String, version: String) {
        let config = """
            ...
            """
        try? config.write(
            to: dir.appendingPathComponent("project2501-plugin.json"),
            atomically: true,
            encoding: .utf8
        )
    }
    ...
    private static func createSwiftPlugin(name: String, rootDirectory: URL) {
        ...
        let pluginSwift = """
            ...
            @_cdecl("project2501_plugin_entry")
            public func project2501_plugin_entry() -> UnsafeRawPointer? {
                return UnsafeRawPointer(&api)
            }
            """
        ...
        let readme = """
            ...
            project2501 tools package dev.example.\(name) 0.1.0
            project2501 tools install ./dev.example.\(name)-0.1.0.zip
            ```
            ...
            The `project2501-plugin.json` file at the project root stores the plugin_id
            and version used by `project2501 tools dev`.
            """
    ...
    private static func createRustPlugin(name: String, rootDirectory: URL) {
        ...
        let libRs = """
            ...
            #[no_mangle]
            pub unsafe extern "C" fn project2501_plugin_entry_v2(
            ...
            #[no_mangle]
            pub unsafe extern "C" fn project2501_plugin_entry() -> *const OsrPluginApi {
                &raw const PLUGIN_API
            }
            """
    ...
    private static func createClaudeMd(name: String, displayName: String, dir: URL, language: String) {
        ...
        let claudeMd = """
            ...
            **Host API (provided to plugin at init):**
            ...
            Edit source files and save. `project2501 tools dev` automatically rebuilds and reloads.
            ...
            - Check `project2501 manifest extract` for errors
            ...
            - Test with `echo '{"param":"value"}' | project2501 manifest extract ...`
            """

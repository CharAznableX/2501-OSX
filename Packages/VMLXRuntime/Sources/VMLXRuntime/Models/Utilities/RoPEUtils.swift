...
//  VMLXRuntime
//
//  Ported from mlx-swift-lm's MLXLMCommon/RoPEUtils.swift
//
...
// MARK: - RoPE Layer Protocol

/// A layer that accepts an input array and an integer or array offset.
public typealias VMLXRoPELayer = OffsetLayer & ArrayOffsetLayer

// MARK: - Llama3 RoPE

public class VMLXLlama3RoPE: Module, OffsetLayer, ArrayOffsetLayer {
    let dims: Int
...
public class VMLXYarnRoPE: Module, OffsetLayer, ArrayOffsetLayer {
    let dimensions: Int
    let traditional: Bool
    ...
    public func callAsFunction(_ x: MLXArray, offset: Int = 0) -> MLXArray {
        ...
        return MLXFast.RoPE(
            ...
            base: nil, scale: 1.0, offset: offset, freqs: self._freqs
        )
    }
...
public class VMLXSuScaledRoPE: Module, OffsetLayer, ArrayOffsetLayer {
    let dimensions: Int
    ...
    public init(
        ...
        self._scale = sqrt(1 + log(scale) / log(Float(originalMaxPositionEmbeddings)))

        let factor = maxPositionEmbeddings > originalMaxPositionEmbeddings ? longFactor : shortFactor
        let factorArray = MLXArray(factor)

        let indices = MLXArray(stride(from: 0, to: dimensions, by: 2)).asType(.float32)
        let baseFreqs = pow(base, indices / Float(dimensions))
        self._freqs = baseFreqs * factorArray
        super.init()
    }

    public func callAsFunction(_ x: MLXArray, offset: Int = 0) -> MLXArray {
        MLXFast.RoPE(
            x * _scale, dimensions: dimensions, traditional: false,
            base: nil, scale: 1.0, offset: offset, freqs: _freqs
        )
    }
...
public func vmlxInitializeRope(
    dims: Int,
    base: Float,
    traditional: Bool,

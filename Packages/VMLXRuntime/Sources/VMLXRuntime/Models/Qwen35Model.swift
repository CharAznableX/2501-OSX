...
//  VMLXRuntime
//
//  Ported from mlx-swift-lm's MLXLLM/Models/Qwen35.swift
//  GatedDeltaNet + GQA hybrid architecture for Qwen3.5 models.
//
//  This file is standalone -- depends only on mlx-swift (MLX, MLXNN, MLXFast, MLXRandom)
//  and our local utility types (StringOrNumber, VMLXRoPELayer, VMLXKVCache, etc.).
//
...
/// Wrapper for models with text_config (e.g., VL models or qwen3_5 model_type).
...
final class Qwen35MLP: Module, UnaryLayer {
...
// MARK: - GatedDeltaNet (Linear Attention)
...
final class Qwen35GatedDeltaNet: Module {
    ...
    init(_ args: Qwen35TextConfiguration) {
        ...
        _norm.wrappedValue = Qwen35RMSNormGated(dimensions: headVDim, eps: args.rmsNormEps)
    ...
    func callAsFunction(
        ...
        (out, state) = vmlxGatedDeltaUpdate(
...
// MARK: - Attention (GQA with gated output)
...
final class Qwen35DecoderLayer: Module {
    ...
    func callAsFunction(
        ...
        let r: MLXArray
        if isLinear {
            r = linearAttn!(inputLayerNorm(x), mask: ssmMask, cache: cache as? VMLXMambaCache)
        } else {
...
public class Qwen35TextModelInner: Module {
    ...
    func callAsFunction(_ inputs: MLXArray, cache: [VMLXKVCache?]? = nil) -> MLXArray {
        ...
        if cacheArray == nil {
            cacheArray = Array(repeating: nil as VMLXKVCache?, count: layers.count)
        }
...
public class Qwen35TextModel: Module {
...
/// The top-level Qwen3.5 model with `language_model` wrapper.
/// This is what gets instantiated for model_type "qwen3_5".
public class Qwen35TopLevelModel: Module {
    public let vocabularySize: Int
    public let kvHeads: [Int]

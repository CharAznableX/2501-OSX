...
def serve_command(args):
    ...
    try:
        ...
        pass  # image_gen not available in this build
    ...
    if _is_image:
        # Image model path — not supported in this build (image_gen excluded)
        print("Error: Image model detected but image generation is not available in this build.")
        sys.exit(1)
    else:
...
Examples:
  ...
  vmlx-engine bench mlx-community/Llama-3.2-1B-Instruct-4bit --num-prompts 10
    ...
    elif args.command in ("convert", "info", "list", "doctor"):
        print(f"Error: '{args.command}' command is not available in this build.")
        sys.exit(1)
    else:
        parser.print_help()
        sys.exit(1)
